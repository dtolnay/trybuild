...
use crate::{features, rustflags, Expected, FileTest, InlineTest, Runner, Test};
    ...
    fn make_manifest(
        ...
        for expanded in tests {
            if expanded.error.is_none() {
                let path = match expanded.test {
                    Test::File(ref t) => source_dir.join(&t.path),
                    Test::Inline(ref t) => project_dir.join(&format!("{}.rs", t.name)),
                };
                manifest.bins.push(Bin {
...
fn expand_globs(tests: &[Test]) -> Vec<ExpandedTest> {
    ...
    for test in tests {
        let expanded = if let Test::File(test) = test {
